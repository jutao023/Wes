...
namespace wes
{
    class SqLiteHelper
    {
        ...
        public SQLiteDataReader Execute(string queryString)
        {
            try
            {
        ...
        /// <summary>
        /// 插入一条数据，不必带主键ID，但是主键必须有自增属性且是第一个字段
        /// </summary>
        /// <returns>The values.</returns>
        /// <param name="tableName">数据表名称</param>
        /// <param name="values">插入的数值</param>
        public SQLiteDataReader InsertValuesNoKey(string tableName, string[] values)
        {
            //获取数据表中字段数目
            int fieldCount = ReadFullTable(tableName).FieldCount;
            //当插入的数据长度不等于字段数目时引发异常
